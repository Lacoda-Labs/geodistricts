...
- **Data and reproducibility**: Use required data sources (Census population, TIGER/Line boundaries, district counts) per GDIP-003 so that results are reproducible and comparable across studies.
...
- **State legislation or commission rules**: A state may adopt the GeoDistricts Protocol by reference (e.g. “redistricting shall follow the GeoDistricts Protocol, version X,” or “districts shall be generated using the algorithm specified in GDIP-004”). Implementers and legislators should:
  - Reference the protocol by **version or release tag** (e.g. “Protocol 1.0” or the set of GDIPs in effect at adoption) so that updates to the protocol do not automatically change state law unless the state chooses to update the reference.
